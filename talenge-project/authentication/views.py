        ...
        if request.method == 'POST':
            ...
            if user:
                ...
                if user.is_active:
                    ...
                    login(request, user)
                    the_user_type = UserProfile.objects.filter(user__username=username)
                    request.session['utype'] = the_user_type[0].type_user
                    return HttpResponseRedirect('/')
                else:
                    # An inactive account was used - no logging in!
                    return HttpResponse("Your Talenge account is disabled.")
            else:
